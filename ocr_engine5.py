...
def get_textract_structured_text(image_bytes: bytes) -> str:
    """Uses Textract analyze_document with FORMS and TABLES to extract structured context."""
    ...
    # Helper to extract text from child WORD blocks
    def get_text(block_or_id):
        if isinstance(block_or_id, dict):
            block = block_or_id
        else:
            block = block_map.get(block_or_id)
    ...
    for block in blocks:
        # 1. Extract Key-Value Pairs (Forms)
